...
<div class="main-right-box">
    ...
    <span class="pull-right">
        ...
        <input class="btn btn-primary" type="button" value=" {lang_admin('user_manage_settings')} " onclick="gotourl(this)"   data-dataurl="{url::create('config/system/set/membersite')}" data-dataurlname="{lang_admin('user_manage_settings')}" />

        <a href="#" onclick="gotourl(this)"   data-dataurl="{$base_url}/index.php?case=table&act=add&table=user&admin_dir={get('admin_dir',true)}&site=default" class="btn btn-gray" data-dataurlname="{lang_admin('adding_users')}">{lang_admin('adding_users')}</a>
            ...
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tr class="th">
                        ...
                        <th class="catid"><!--nickname-->{lang_admin('nickname')}</th>
                        <?php if(session::get('ver') == 'corp'){ ?>
                            <th class="catid text-center"><!--groupid-->{lang_admin('integral')}</th>
                        <?php } ?>
                        <th class="menoy"><!--groupid-->{lang_admin('balance')}</th>
                        <th class="catid"><!--groupid-->{lang_admin('user_group')}</th>
                    ...
                    <tr>
                        ...
                        <td class="catid">{cut($d['nickname'])}</td>
                        <?php if(session::get('ver') == 'corp'){ ?>
                            <td class="text-center">{usergroupisadministrator($d['groupid'])?'无':cut($d['integration'])}</td>
                        <?php } ?>
                        <td class="catid">{$d['menoy']}</td>
                        <td class="catid">{usergroupname($d['groupid'])}</td>
