...
<h5>{lang_admin('comment_list')}</h5>
<div class="blank20"></div>
<div class="box" id="box">
    <input class="btn btn-primary" type="button" value=" {lang_admin('comment_manage_settings')} " onclick="javascript:window.location.href='{url::create('config/system/set/discuss')}'" />
    <div class="blank20"></div>
...
<th class="htmldir">{lang_admin('state')}</th>
...
<td class="htmldir">{if $d['state']}<i class="glyphicon glyphicon-ok"></i>{else}<i class="glyphicon glyphicon-remove"></i>{/if}</td>
...
<a onclick="if(confirm('{lang_admin('are_you_sure_you_want_to_delete_it')}')){gotourl(this);};" href="#"
   data-dataurl="<?php echo modify("/act/delete/table/$table/id/$d[$primary_key]/token/$token");?>" title="{lang_admin('delete')}"><i class="glyphicon glyphicon-trash"></i></a>
</td>
</tr>
{/loop}
...
<input type="hidden" name="batch" value="">
   <input  class="btn btn-secondary" type="button" value=" {lang_admin('delete')} " name="delete" onclick="if(getSelect(this.form) && confirm('{lang_admin('do_delete_id_as')}('+getSelect(this.form)+'){lang_admin('is_it_recorded')}')){this.form.action='<?php echo modify('act/batch',true);?>'; this.form.batch.value='delete'; returnform(this.form);}"/>

    <input class="btn btn-primary" type="button" value=" {lang_admin('to_examine')} " name="docheck" onclick="if(getSelect(this.form) && confirm('{lang_admin('do_audit_id_for')}('+getSelect(this.form)+'){lang_admin('is_it_recorded')}')){this.form.action='<?php echo modify('act/batch',true);?>'; this.form.batch.value='docheck'; returnform(this.form);}"/>

<input class="btn btn-success" type="button" value="{lang_admin('cancellation_of_audit')}" name="douncheck" onclick="if(getSelect(this.form) && confirm('{lang_admin('do_cancel_the_audit_id_for')}('+getSelect(this.form)+'){lang_admin('is_it_recorded')}')){this.form.action='<?php echo modify('act/batch',true);?>'; this.form.batch.value='douncheck'; returnform(this.form);}"/>

<input  class="btn btn-danger" type="button" value=" {lang_admin('empty')} " name="clearall" onclick="if(confirm('{lang_admin('do_you_really_want_to_empty')}')){this.form.action='<?php echo modify("/act/clearall/table/$table/token/$token");?>'; this.form.batch.value='clearall'; returnform(this.form);}"/>
