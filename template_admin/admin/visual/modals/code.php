<div class="modal fade code-config" id="codeModel" tabindex="-1" role="dialog"
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <textarea rows="50" id="codecontent" style="height: 300px;" placeholder="{lang_admin('enter_the_code_here')}" name="codecontent" type="text" class="form-control"></textarea>
                </div>
            </div>
    ...
    <label>
      <input type="checkbox" id="code-checkbox" name="code-checkbox"> {lang_admin('hide')}
    </label>
  </div>
                <button  type="button" class="btn btn-default" data-dismiss="modal">
                    {lang_admin('close')}
                </button>
                <button id="btn_saveCode" type="button" class="btn btn-primary" data-dismiss="modal">{lang_admin('preservation')}
                </button>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('body.edit .visual-right').on("click", "[data-target='#codeModel']", function (e) {
            e.preventDefault();
            codeHandel = $(this).parent().parent().parent().find('.view .visual-code');
