		...
		success: function (data) {
			...
			for(var index=0;index<datanamefield.length;index++){
				if (archiveData[datanamefield[index]] == 'num'|| archiveData[datanamefield[index]]==''
                    ...
                    if(archiveData[datanamefield[index]].indexOf("\r\n") >= 0 ) {
                        var dataarray=archiveData[datanamefield[index]].split("\r\n");
                    } else if(archiveData[datanamefield[index]].indexOf("\n") >= 0 ) {
                        var dataarray=archiveData[datanamefield[index]].split("\n");
                    }
	...
	if(shopingtype.length==0){
		...
		if (ischunzai){
			...
			if(shopingtype.length==0 || oldlightgallerylode.length==0){
				$(".swiper-shop-pic").html(oldlightgallery[0]);
			}else{
				//清空相同类型的
				...
				$(".swiper-shop-pic").html(oldlightgallery[oldlightgallerylode[oldlightgallerylode.length-1]]);
			}
		}
	}
...
function  setlightgallery(imageurl,val,type){
	if (imageurl !=""){
		if (oldlightgallery.length==0){
			oldlightgallery[0]=$(".swiper-shop-pic").html();
		}
		if($.inArray(type+","+val,oldlightgallerylode)>=0){
			oldlightgallerylode.splice($.inArray(type+","+val,oldlightgallerylode),1);
		}
		var htmlstr="<div id=\"lightgallery\" role=\"listbox\" class=\"swiper-wrapper\"><div class=\"swiper-slide swiper-slide-duplicate\" data-src=\""+imageurl+"\" data-sub-html=\"\" data-swiper-slide-index=\"1\" lg-event-uid=\"\"><div class=\"show_products1-item\">\n" ;
		htmlstr+="<a href=\"\" rel=\"lightbox['roadtrip']\"><img src=\""+imageurl+"\" alt=\""+val+"\" /></a>\n";
		htmlstr+="</div></div></div>";
