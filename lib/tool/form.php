...
class form
{
    ...
    function hidden($name, $value = null, $option = null,$check = null)
    {
    ...
    function textarea($name, $value = null, $option = null,$check = null)
    {
...
$(document).ready(function()
    ...
    function getform($name, $form, $field, $data,$check=null)
    {
		//var_dump($name);
        ...
        elseif (@$form[$name]['selecttype'] == 'select') {
            //var_dump($form[$name]);
            if($form[$name][$newselectname]!=""){    //区分表单的多选  和  内容的多选
    ...
    static function upload_thumb($name, $value)
    {
        ...
        <a title=\"".lang('select_files')."\" onclick=\"javascript:windowsdig('".lang('select_files')."','iframe:index.php?case=file&act=updialog&fileinputid={$name}&getbyid={$name}_preview&max=1&checkfrom=picshow&admin_dir=".config::getadmin('admin_dir')."','900px','480px','iframe')\" href=\"#body\"><span id=\"{$name}_preview\"><img src=\"$img_url\" class=\"img-responsive\" onerror='this.src=\"common/js/ajaxfileupload/pic.png\"' /></span></a></div><div class=\"blank10\"></div><div class=\"thumb-box\"><input name=\"$name\"  id=\"$name\" value=\"$value\"  class=\"form-control\" />" .
            '<div class="blank10"></div><input value=" '.lang('delete').' " class="btn btn-gray  thumb-del" title="'.lang('delete').'" id="' . $name . '_del" onclick="pics_delete(\'\',\'' . $name . '\');document.getElementById(\'' . $name . '_preview\').innerHTML=\'<img src=\\\'common/js/ajaxfileupload/pic.png\\\' style=\\\'max-width:90px\\\'>\';" value="'.lang('delete').'" type="button" name="delbutton" class="btn btn-gray" /></div>';
...
EOT;*/
    ...
    static function getuploadhtml($i, $name, $purl, $value)
    {
        $cname = $name;
        $name = $name . $i;
        $res = '<div id="' . $name . '_up"><span id="' . $name . '_preview"></span><br><br>'.lang('address').'：<input name="' . $name . '" id="' . $name . '" value="' . $value . '" size="50"/> <input type="button" id="' . $name . '_del" class=\"btn btn-gray\" name="delbutton" value="'.lang('delete').'" onclick="pics_delete(' . $i . ',\'' . $cname . '\');" style="display:;"><br><br>';
    ...
    static function upload_image2($name, $value)
    {
        ...
        if (front::$act == 'edit' && $value) {
            ...
            $res .= "<div id=\"pics0_up\"><span id=\"{$name}0_preview\"></span><div class=\"blank10\"></div><input name=\"{$name}0\"  id=\"{$name}0\" value=\"$value\"  class=\"form-control\" /> <input type=\"button\" id=\"{$name}0_del\" name=\"delbutton\" value=\"".lang('delete')."\" onclick=\"pics_delete('0','{$name}');\" class=\"btn btn-gray\" style=\"display:none;\">";
