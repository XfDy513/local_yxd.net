...
class ueditor_act extends act {
    ...
    function upload(){
        ...
        if(htmlspecialchars($_GET['action'])=="uploadimage"){
                $fileurl=$up->getFileInfo();
                $fileurl=$fileurl['url'];
                $info = pathinfo($fileurl);
                $file_name =  basename($fileurl,'.'.$info['extension']);   //获取图片名称
                $file_hou_name = array_pop(explode('.',$fileurl)); //获取图片后缀

            //图片上传  插件
            apps::updateimg("images/".date("Ymd")."/".$file_name.'.'.$file_hou_name,ROOT.$fileurl);
        }
        /**
         * 得到上传文件所对应的各个参数,数组结构
         * array(
    ...
    {
        $files[] = array();
        if(isset(front::$user) && front::$user['groupid'] == '2') {
        ...
        if (isset($_GET["callback"])) {
            if (preg_match("/^[\w_]+$/", $_GET["callback"])) {
                ...
                echo json_encode(array(
                    'state'=> 'callback'.lang('illegal_parameter')
                ));
            }
        } else {
            echo $result;
