...
class user_act extends act
{
    function init()
    {
        ...
        $nologin_arr = array('login', 'ologin', 'respond', 'dialog_login',
            'space', 'register', 'login_js', 'login_success', 'getpass','cmseaylogin');
    ...
    function index_action()
    {
        ...
        if(file_exists(ROOT."/lib/table/vhost.php")) {
            $where = "uid={$this->cur_user['userid']}";
            $vhost_count = vhost::getInstance()->getrows($where, 0, 'id desc', '*');
            $this->view->vhost_count = count($vhost_count);
        }
    }
    ...
    function login_action()
    {
        ...
        if (!$this->loginfalsemaxtimes())
            if (front::post('submit')) {
                ...
                if ((config::get('mobilechk_enable') && config::get('mobilechk_login')) || (front::post('dxdlnum') && front::post('dxdlnum')=='1')) {
    ...
    {
        if (cookie::get('loginfalse') >= config::get('template_nologin') || event::loginfalsemaxtimes()) {
            ...
            if (!empty($user['answer'])) {      //安全问题
                ...
                if(front::post('getpass_type')=="0"){  //是邮箱的时候
                        ...
                        if (!$smsCode->chkcode($mobilenum)) {
                            ...
                            echo template('getpass_newpass.html');
                        }
                }
            }
        }
