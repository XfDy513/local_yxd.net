...
class guestbook_act extends act
{
    ...
    {
        if(!config::get('guestbook_enable')){
            exit(lang('message_feature_to_turn_off'));
        }
            $this->list_action();
        if (front::post('submit')) {
            ...
            foreach($arr as $key=>$value){
                if( strpos(front::post('content'),$value) || strpos(front::post('title'),$value)|| strpos(front::post('nickname'),$value)){
                    $a=1;
                }
            }
            if($a==1){
                alerterror(lang('不能包含特殊字符').front::post('content'));
                exit;
            }
          ...
          /*  if (!front::post('guesttel')) {
            ...
            if ($insert < 1) {
                ...
                $body = lang('title').':'.front::$post['title'].
                    '<br>'.lang('lang_name').':'.front::$post['username'].
                    '<br>'.lang('lang_phone').':'.front::$post['guesttel'].
                    '<br>'.lang('lang_email').':'.front::$post['guestemail'].'<br>'.lang('lang_qq').':'.front::$post['guestqq'].
                    '<br>'.lang('content').':'.front::$post['content'];
    ...
    function list_action()
    {
        ...
        $where = "checked=1 and (( userid<>'". user::getusersid()."' and issee=0 ) or (userid='".user::getusersid()."'))";
        ...
        front::$record_count = $this->_table->record_count;
