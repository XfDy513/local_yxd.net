   ...
   /* protected $_table;*/
    ...
    function batch_action(){
        ...
        if(isset($_GET['logTime'])){
            ...
            $filename  = 'data/event/'.$fileToName;//定义操作文件
    ...
    function deleteLogTxt_action(){
         if(isset($_GET['deletType'])){
             $path = ROOT."/data/event/";  //设置需要删除的文件夹
             $p = scandir($path);
             if($_GET['deletType']=='0'){  //删除本日
                 foreach($p as $val){
                     //排除目录中的.和..
                     $filetime=date('Ymd', time());
    ...
    function delete_action(){
      ...
      if(isset($_GET['id'])){
           ...
           //front::refresh(url('adminlogs/manage'));
      }
