...
class field_admin extends admin {
    ...
    function list_action() {
        ...
        foreach ($fields as $key => $value){
         ...
         /*   if( (!setting::$var[$this->table][$key]['langname']) ||
                ...
                $fields[$key]['listorder']=setting::$var[$this->table][$key]['listorder'];
                $newArr[$key]['listorder']=setting::$var[$this->table][$key]['listorder'];
          /*  }else{
                unset($fields[$key]);
            }*/
        }
        array_multisort($newArr,SORT_ASC,$fields);
    ...
    function add_action() {
        ...
        //新增标签
        if(front::$post['batchmediumtext']=='addmediumtext') {
        ...
        if (front::post('batch') == 'listorder') {
            ...
            if (is_array($fields))
                foreach ($fields as $id => $field) {
                    foreach(setting::$var[$this->table][$id] as $k=>$n) {
                      /*  if(isset(setting::$var[$this->table][$id]['listorder'])){*/
                            setting::$var[$this->table][$id]['listorder']=intval($field);
                      /*  }*/
                    }
                    setting::save();
                }
    ...
    function getfieldcatid_action()
    {
        ...
        if (is_array($langdata)) {
            foreach ($langdata as $key => $value) {
                $option= array(0 => '请选择...');
                if ($_GET['isshopcatid']) {
                    $catdata = category::optionshopping('0', 'tolast', $option, $level, $value['langurlname']);
                } else {
                    //$catdata= category::optionconnent(0,'tolast');
                    $catdata = category::optionall(0, 'tolast', $option, $level, $value['langurlname']);
                }
                $select = "";
