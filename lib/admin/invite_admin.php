...
class invite_admin extends admin{
    ...
    function add_action(){
        ...
        front::redirect(url('invite/list', true));
        exit;
    }
    ...
    function batch_action(){
        if (front::post('batch') && front::post('select')) {
            ...
            if (front::post('batch') == 'delete') {
                ...
                if ($delete > 0) {
                    front::flash(lang_admin('delete').lang_admin('success'));
                    event::log(lang_admin('delete').lang_admin('invitation_code'), lang_admin('success').' id=' . implode(',', front::post('select')));
                } else
                    front::flash(lang_admin('no_record_was_deleted'));
            }
        }
