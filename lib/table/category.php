...
class category extends table
{
    ...
    function get_form()
    {
        return array(
            'parentid' => array(
                'selecttype' => 'select',
                'select' => form::arraytoselect(category::getoptionconnent(0, 'isnotlast')),
    ...
    function get_form_field()
    {
        return array(
            'parentid' => array(
                'selecttype' => 'select',
                'select' => form::arraytoselect(category::optionconnent(0, 'isnotlast')),
                'default' => get('parentid'),
            ),
            'langid'=>array(
                'selecttype'=>'select',
        ...
        if (cache::get($cache_id))
            ...
            foreach($_category as $key=>$category){
                ...
                if ($category['nofollow']){
                    $category['nofollow'] = '" rel="nofollow';
                }else{
                    $category['nofollow'] = '';
                }
                if ($category['isblank']){
                    $category['isblank'] = '" target="_blank';
                }else{
                    $category['isblank'] = '';
                }
                $_category[$key]['url'] = category::url($category['catid']).$category['nofollow'].$category['isblank'];
    ...
    function son($id, $langurlname = '')
    {
        ...
        $_category = category::getInstance()->getrows($where, 1000, 'listorder=0,listorder asc');
        $category = array();
        foreach ($_category as $one) {
            if (!front::$admin && !$one['isshow']) continue;
            $category[$one['catid']] = $one;
    ...
    static function optionall($catid = 0, $tag = 'all', &$option = array(0 => '请选择...'), &$level = 0,$langurlname = '')
        {
            $category = self::getInstance();
            if (is_array($category->son($catid,$langurlname))) foreach ($category->son($catid,$langurlname) as $_catid) {
                if (!self::check($_catid, $tag)) continue;
                $strpre = $level > 0 ? str_pad('', $level * 12, '&nbsp;') . '└&nbsp;' : '';
                $option[$_catid] = $strpre . $category->category[$_catid]['catname'];
                if (is_array($category->son($_catid,$langurlname))) {
    ...
    {
        $category = self::getInstance();
        if (is_array($category->sonshopping($catid,$langurlname))) foreach ($category->sonshopping($catid,$langurlname) as $_catid) {
            if (!self::check($_catid, $tag)) continue;
            $strpre = $level > 0 ? str_pad('', $level * 12, '&nbsp;') . '└&nbsp;' : '';
            $option[$_catid] = $strpre . $category->category[$_catid]['catname'];
    ...
    {
        $_category = $this->getrows('isshopping=1 or parentid <> "0"', 1000, 'listorder=0,listorder desc');
        $category = array();
        $_lang =new lang();
        $langurlname=$langurlname==""?lang::getisadmin():$langurlname;
        $_langdata= $_lang->getrows('langurlname="'.$langurlname.'"', 1, 'id asc');
    ...
    static function gettemplate($catid, $tag = 'listtemplate', $up = true,$_style=null,$archive=false)
    {
        ...
        if (@$category->category[$catid][$tag] && ($_style==null ||  file_exists(TEMPLATE . '/' . $_style . '/' . $category->category[$catid][$tag])
                || (file_exists(TEMPLATE . '/'.$category->category[$catid][$tag]))  || (file_exists(TEMPLATE .'/'.config::get('template_shopping_dir'). '/'.$category->category[$catid][$tag])) )) return $category->category[$catid][$tag];
        if (!$up) return;
    ...
    public static function gethtmlcategorydata_new($tree,$children=false,$leveldiv=0,$token=""){
        ...
        foreach($tree as $t)
        {
            if($t['children'] == '')
            {
                ...
                $html .= '</select></span></td>';
                $html .= '<td class="htmldir text-center">';
