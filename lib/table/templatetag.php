...
class templatetag
{
    ...
    function __construct()
    {
        ...
        if (!file_exists($settingTemplate_file))
            ...
            if (count( self::$settingTemplate)<=0){
    ...
    public function rec_insertview($tag_info)
    {
        ...
        $max_id = 0;
        foreach (self::$setting as $set) {
            if ($set['name'] == $tag_info['name'] && $set['name']!="") {
                front::flash(lang_admin('tags')." $tag_info[name]".lang_admin('already_exists'));
                return false;
            }
            if ($set['id'] > $max_id)
    ...
    static function tag($tagid)
    {
      ...
      /*  $templatetag=new templatetag();
        ...
        if (is_array($tag)) {
            ...
            if ($tag['tagfrom'] == 'category' || $tag['tagfrom'] == 'content'|| $tag['tagfrom'] == 'type'|| $tag['tagfrom'] == 'shoptype'
                || $tag['tagfrom'] == 'special'|| $tag['tagfrom'] == 'shopspecial'  || $tag['tagfrom'] == 'shopcontent' || $tag['tagfrom'] == 'shopcategory'
                || $tag['tagfrom'] == 'announcement'){
            ...
            return front::$view->_eval($content);
        }
        return "";
    }
