...
class plugins
{
           ...
           return cache::get($cache_id);
            ...
            if ($id == 0) {
                ...
                foreach ($categories as $catid) {
                    ...
                    $_category['url'] = category::url($_category['catid'],null,false,lang::getistemplate());
            ...
            $catinfo[0]['url'] = category::url($id,null,false,lang::getistemplate());
    ...
    {
        //var_dump($_len);
        $cache_id =lang::getistemplate().'/special/'.$id.'/specialinfo-'.$id.'-'.$_subtitle.'-'.$_spcontent.'-'.$_len.'-'.$_spimage;
        if (cache::get($cache_id))
            return cache::get($cache_id);
        else {
    ...
    public static function typeinfo($id, $_tyname = '1', $_subtitle = '1', $_tycontent = '1', $_len = 0, $_tyimage = 1)
    {
        ...
        if (cache::get($cache_id))
            ...
            if (is_array($rows) && !empty($rows)) {
                foreach ($rows as $k => $row) {
                    $row['url'] = type::url($row['typeid'], $row['ishtml']);
                    //var_dump($row);
                    if ($_tycontent && $_len > 0) {
                        //var_dump($row);
